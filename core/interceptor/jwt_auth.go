...
func valid(ctx context.Context, authorization []string) (context.Context, error) {
	if len(authorization) < 1 {
		return ctx, exception.NewAuthError(ctx, "AUTH_404", "auth token required")
	}
	...
	if err != nil || !token.Valid {
		...
		return ctx, exception.NewAuthError(ctx, "AUTH_401", "invalid auth token")
	}
	// save current user
	return security.SaveCurrentUser(ctx, security.NewPrinciple(token)), nil
}
